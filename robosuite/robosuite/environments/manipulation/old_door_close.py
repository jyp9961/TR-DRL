...
class Old_Door_Close(SingleArmEnv):
    """
    This class corresponds to the door closing task for a single robot arm.
    ...
    def reward(self, action=None):
        ...
        Sparse un-normalized reward:
            ...
            - a discrete reward of 1.0 is provided if the door is closed
        ...
        # else, we consider only the case if we're using shaped rewards
            ...
            # Add grasping handle
            check_grasp = self._check_grasp(gripper=self.robots[0].gripper, object_geoms=self.door.important_sites["handle"])
            if check_grasp:
                reward += 0.25
    ...
    def _load_model(self):
        ...
        self.door = DoorObject(
            ...
            damping=0.1,
            lock=self.use_latch,
            xml_path="objects/old_door_lock.xml"
        )
    ...
    def _reset_internal(self):
        ...
        if not self.deterministic_reset:
            ...
            # initialize the door to be open
            self.sim.data.qpos[self.hinge_qpos_addr] = 0.3

    def _check_success(self):
        """
        Check if door has been closed.

        Returns:
            bool: True if door has been closed
        """
        check_grasp = self._check_grasp(gripper=self.robots[0].gripper, object_geoms=self.door.important_sites["handle"])
        hinge_qpos = self.sim.data.qpos[self.hinge_qpos_addr]
        handle_qpos = self.sim.data.qpos[self.handle_qpos_addr]
        # return check_grasp and hinge_qpos < 0.01
        return hinge_qpos < 0.01 and np.abs(handle_qpos) < 0.05
