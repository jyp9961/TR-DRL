...
from moore.algorithms.actor_critic import MTSAC, TRMTSAC
...
def run_experiment(args, save_dir, exp_id = 0, seed = None):
    ...
    if exp_type == 'MT28':
        benchmark = getattr(utils, exp_type)()
    else:
    ...
    if exp_type == 'MT10':
        ...
        # 'drawer-close-v2', 'button-press-topdown-v2', 'peg-insert-side-v2', 'window-open-v2', 'window-close-v2' 
        # drawer-open-v2 4; drawer-close-v2 5; window-open-v2 8; window-close-v2 9
        reversible_indices = [4, 5, 8, 9]
        reversible_dict = {4:5, 5:4, 8:9, 9:8}
        dynamics_to_env_dict = {0:[4, 5], 1:[8, 9]}
        env_to_dynamics_dict = {4:0, 5:0, 8:1, 9:1}
    if exp_type == 'MT50':
        ...
        # 'handle-pull-side-v2', 'handle-pull-v2', 'lever-pull-v2', 'peg-insert-side-v2', 'pick-place-wall-v2', 
        # 'pick-out-of-hole-v2', 'reach-v2', 'push-back-v2', 'push-v2', 'pick-place-v2', 
        # 'plate-slide-v2', 'plate-slide-side-v2', 'plate-slide-back-v2', 'plate-slide-back-side-v2', 'peg-unplug-side-v2', 
        # 'soccer-v2', 'stick-push-v2', 'stick-pull-v2', 'push-wall-v2', 'reach-wall-v2', 
        # 'shelf-place-v2', 'sweep-into-v2', 'sweep-v2', 'window-open-v2', 'window-close-v2'
        # assembly-v2 0; disassemble-v2 12;
        # coffee-pull-v2 9; coffee-push-v2 10;
        # door-open-v2 15; door-close-v2 13;
        # door-lock-v2 14; door-unlock-v2 16;
        # drawer-open-v2 19; drawer-close-v2 18;
        # faucet-open-v2 20; faucet-close-v2 21;
        # handle-press-v2 24; handle-pull-v2 26;
        # handle-press-side-v2 23; handle-pull-side-v2 25;
        # peg-insert-side-v2 28; peg-unplug-side-v2 39;
        # plate-slide-v2 35; plate-slide-back-v2 37;
        # plate-slide-side-v2 36; plate-slide-back-side-v2 38;
        # push-v2 33; push-back-v2 32;
        # stick-push-v2 41; stick-pull-v2 42;
        # window-open-v2 48; window-close-v2 49;
        # reach-v2 31;
        # pick-place-v2 34;

        reversible_indices = [
            0, 12,   # assembly <-> disassemble
            9, 10,   # coffee-pull <-> coffee-push
            13, 15,  # door-close <-> door-open
            14, 16,  # door-lock <-> door-unlock
            18, 19,  # drawer-close <-> drawer-open
            20, 21,  # faucet-open <-> faucet-close
            23, 25,  # handle-press-side <-> handle-pull-side
            24, 26,  # handle-press <-> handle-pull
            28, 39,  # peg-insert-side <-> peg-unplug-side
            35, 37,  # plate-slide <-> plate-slide-back
            36, 38,  # plate-slide-side <-> plate-slide-back-side
            32, 33,  # push-back <-> push
            41, 42,  # stick-push <-> stick-pull
            48, 49,   # window-open <-> window-close
            31, # reach-v2 <-> reach-v2
            34 # pick-place-v2 <-> pick-place-v2
        ]
        reversible_dict = {
            0: 12, 12: 0,
            9: 10, 10: 9,
        ...
        # Group reversibles by type of dynamics (arbitrary grouping for example)
        dynamics_to_env_dict = {
            0: [0, 12],         # assembly/disassemble
            1: [9, 10],         # coffee-pull/push
            2: [13, 15],        # door-close/open
            3: [14, 16],        # door-lock/unlock
            4: [18, 19],        # drawer-close/open
            5: [20, 21],        # faucet-open/close
            6: [23, 25],        # handle-press-side/pull-side
            7: [24, 26],        # handle-press/pull
            8: [28, 39],        # peg-insert-side/unplug-side
            9: [35, 37],        # plate-slide/back
            10: [36, 38],       # plate-slide-side/back-side
            11: [32, 33],       # push-back/push
            12: [41, 42],       # stick-push/pull
            13: [48, 49],       # window-open/close
            14: [31],           # reach-v2
            15: [34]            # pick-place-v2
        }
        env_to_dynamics_dict = {}
        for dyn, envs in dynamics_to_env_dict.items():
            for env in envs:
                env_to_dynamics_dict[env] = dyn

    # create SAC agent
    env_names = mdp.get_attr("env_name")
    agent = TRMTSAC(env_names = env_names,
                    reversible_indices = reversible_indices,
                    reversible_dict = reversible_dict,
                    dynamics_to_env_dict = dynamics_to_env_dict,
                    env_to_dynamics_dict = env_to_dynamics_dict,
                    mdp_info=mdp.info,
    ...
    result_df = pd.DataFrame(columns = ['step', 'c', 'env_name', 'min_J', 'max_J', 'mean_J', 'mean_discounted_J', 'success_rate', 'epoch_time', 'total_time'])
    ...
    if args.start_epoch == 0:
        ...
        np.save(os.path.join(save_dir, f"reversible_ratio.npy"), core.agent.reversible_ratio)
        ...
        for c, key in enumerate(env_names):
            ...
            result_df.loc[len(result_df.index)] = [args.start_epoch*n_steps, c, key, min_J, max_J, mean_J, mean_discounted_J, success_rate, time.time()-start, time.time()-START]
            result_df.to_csv(os.path.join(save_dir, f"result.csv"))
    ...
    for n in trange(args.start_epoch, n_epochs):
        ...
        for c, key in enumerate(env_names):
            ...
            min_J, max_J, mean_J, mean_discounted_J, success_rate = get_stats(dataset, gamma, gamma_eval, dataset_info=dataset_info)
            result_df.loc[len(result_df.index)] = [(n+1)*n_steps, c, key, min_J, max_J, mean_J, mean_discounted_J, success_rate, time.time()-start, time.time()-START]
            result_df.to_csv(os.path.join(save_dir, f"result.csv"))
...
if __name__ == '__main__':
    ...
    alg_name = "TR_mixture_orthogonal_experts" if args.orthogonal else "TR_mixture_experts"
    # logging
    ...
    logger.strong_line()
    logger.info('Experiment Algorithm: ' + TRMTSAC.__name__)
