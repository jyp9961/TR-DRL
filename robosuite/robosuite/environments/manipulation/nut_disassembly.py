...
class NutDisAssembly(SingleArmEnv):
    """
    ...
    def staged_rewards(self):
        ...
        if self.single_object_mode == 2 and self.nut_id == 1:
            check_grasp = self._check_grasp(
                        gripper=self.robots[0].gripper,
                        object_geoms=self.nuts[self.nut_id].contact_geoms[8],
                    )

            handle_object = self.nuts[1].important_sites["handle"]
            handle_pos = self.sim.data.get_site_xpos(handle_object)
        
            eef_pos = self.sim.data.site_xpos[self.robots[0].eef_site_id]
            peg_pos = np.array(self.sim.data.body_xpos[self.peg2_body_id])
            nut_pos = np.array(self.sim.data.body_xpos[self.obj_body_id[self.nuts[self.nut_id].name]])
            dist_nut_to_goal = np.linalg.norm(nut_pos - self.goal_pos)

        # reaching reward governed by distance to closest object
        dist_eef_to_handle = np.linalg.norm(eef_pos - (handle_pos + np.array([0, 0, 0.02])))
        r_reach = (1 - np.tanh(10.0 * dist_eef_to_handle)) * reach_mult
        # If the gripper is not grasping the nut handle, the agent receives reaching nut reward.
        if not check_grasp: return r_reach

        # reaching goal reward for putting the nut on the goal
        # The agent receives reaching goal reward.
        r_goal = grasp_mult + (goal_mult - grasp_mult) * (1 - np.tanh(dist_nut_to_goal * 10))
    ...
    def _load_model(self):
        ...
        if self.placement_initializer is None:
            ...
            for nut_name, default_y_range in zip(nut_names, ([0.0999, 0.1001], [-0.1001, -0.0999])):
                self.placement_initializer.append_sampler(
                    sampler=UniformRandomSampler(
                        name=f"{nut_name}Sampler",
                        x_range=[0.0999, 0.1001],
    ...
    def _reset_internal(self):
        ...
        self.goal_pos = np.array([-0.15, -0.15, 1.0])
...
class NutDisAssemblySingle(NutDisAssembly):
    """
...
class NutDisAssemblySquare(NutDisAssembly):
    """
...
class NutDisAssemblyRound(NutDisAssembly):
    """
