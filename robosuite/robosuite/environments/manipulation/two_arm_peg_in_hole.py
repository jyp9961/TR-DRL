...
class TwoArmPegInHole(TwoArmEnv):
    ...
    def reward(self, action=None):
        ...
        if self.reward_shaping:
            ...
            # if the hole is not stable or there is contact between peg and hole, only hole stable reward
            if dist_hole_to_init > 0.02:
                return hole_stable_mult * (1-np.tanh(dist_hole_to_init*50))
            
            t, d, cos = self._compute_orientation()
            gripper_site_pos = self.sim.data.body_xpos[self.peg_body_id]
            dist_eef_to_hole = np.linalg.norm(gripper_site_pos - hole_pos)
            # reaching reward
            reaching_reward = 1 - np.tanh(1.0 * dist_eef_to_hole)
            
            # Orientation reward
            orientation_reward = (1 - np.tanh(d)) +  (1 - np.tanh(np.abs(t))) + cos

            reward = hole_stable_mult + (reach_hole_mult - hole_stable_mult) * (reaching_reward + orientation_reward) / 4
    ...
    def _reset_internal(self):
        ...
        self.goal_pos = self.sim.data.body_xpos[self.hole_body_id]
    ...
    def _check_success(self):
        ...
        return d < 0.06 and -0.12 <= t <= 0.14 and cos > 0.95 and dist_hole_to_init < 0.02 and (not check_contact_peg_hole)
