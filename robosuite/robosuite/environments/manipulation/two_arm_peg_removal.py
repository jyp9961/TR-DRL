...
class TwoArmPegRemoval(TwoArmEnv):
    """
    ...
    def reward(self, action=None):
        ...
        hole_stable_mult = 0.25
        out_mult = 0.5
        reach_goal_mult = 1.0
        if self.reward_shaping:
            peg_pos = self.sim.data.body_xpos[self.peg_body_id]
            hole_pos = self.sim.data.body_xpos[self.hole_body_id]
            dist_hole_to_init = np.linalg.norm(hole_pos - self.hole_init_pos)
            check_contact_peg_hole = self.check_contact(self.peg, self.hole)
            t, d, cos = self._compute_orientation()

            # if hole is not stable or there is contact between peg and hole, only hole stable reward
            if dist_hole_to_init > 0.02 or check_contact_peg_hole:
                return hole_stable_mult * (1-np.tanh(dist_hole_to_init*50))

            # if the peg is stable, no contact between peg and hole, and the peg is not out of the hole, the agent receives moving outside reward
            if -0.12 < t <= 0.14:
                reward = hole_stable_mult + (out_mult - hole_stable_mult) * ((t - (-0.12)) / 0.26)
                return reward

            # if the peg is out of the hole, the agent receives goal reaching reward
            dist_peg_to_goal = np.linalg.norm(peg_pos - self.goal_pos)
            reward = out_mult + (reach_goal_mult - out_mult) * (1 - np.tanh(dist_peg_to_goal * 10))
    ...
    def _load_model(self):
        ...
        self.robots[0].init_qpos = np.array([-np.pi * 0.15, np.pi * 0.45, 0, np.pi * 0.45, np.pi * 0.4, np.pi * -0.45, 0])
    ...
    def _reset_internal(self):
        ...
        self.goal_pos = np.array([-0.2, -0.25, 0.95])
        self.hole_init_pos = self.sim.data.body_xpos[self.hole_body_id]
        self.robots[0].sim.data.qpos[self.robots[0]._ref_joint_pos_indexes] = self.robots[0].init_qpos
    ...
    def _check_success(self):
        ...
        return dist_peg_to_goal < 0.05 and dist_hole_to_init < 0.02 and (not check_contact_peg_hole)
