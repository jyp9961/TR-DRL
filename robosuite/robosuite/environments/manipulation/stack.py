...
class Stack(SingleArmEnv):
    ...
    def _reset_internal(self):
        ...
        if not self.deterministic_reset:
            ...
            cubeA_x, cubeA_y = -0.05, 0
            cubeB_x, cubeB_y = 0.05, 0
            self.initial_cubeA_xy = np.array([cubeA_x, cubeA_y])
            self.initial_cubeB_xy = np.array([cubeB_x, cubeB_y])
            self.goal_pos = np.array([cubeB_x, cubeB_y, 0.85])
            
            # reset cubeA, cubeB
            self.sim.data.set_joint_qpos(self.cubeA.joints[0], np.concatenate([self.initial_cubeA_xy, [table_height + 0.05], obj_quat]))
