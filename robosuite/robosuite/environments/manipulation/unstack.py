...
class UnStack(SingleArmEnv):
    ...
    def _reset_internal(self):
        ...
        if not self.deterministic_reset:
            ...
            obj_quat = np.array([0, 1, 1, 0])
            table_height = 0.8
           
            goal_posx, goal_posy = -0.05, 0
            self.goal_pos = np.array([goal_posx, goal_posy, 0.85])
            
            cubeB_x, cubeB_y = 0.05, 0
            cubeA_x, cubeA_y = cubeB_x, cubeB_y
            self.initial_cubeA_xy = np.array([cubeA_x, cubeA_y])
            self.initial_cubeB_xy = np.array([cubeB_x, cubeB_y])            

            # reset cubeA, cubeB
            self.sim.data.set_joint_qpos(self.cubeA.joints[0], np.concatenate([self.initial_cubeA_xy, [table_height + 0.1], obj_quat]))
            self.sim.data.set_joint_qpos(self.cubeB.joints[0], np.concatenate([self.initial_cubeB_xy, [table_height + 0.05], obj_quat]))
    ...
    def _check_success(self):
        ...
        cubeA_pos = self.sim.data.body_xpos[self.cubeA_body_id]
        cubeB_pos = self.sim.data.body_xpos[self.cubeB_body_id]
        check_grasp_cubeA = self._check_grasp(gripper=self.robots[0].gripper, object_geoms=self.cubeA)

        success = False
        cubeB_not_move = np.linalg.norm(cubeB_pos[0:2] - self.initial_cubeB_xy) < 0.05
        cubeA_on_goal = np.linalg.norm(cubeA_pos - self.goal_pos) < 0.05
